 ...
 */
    private boolean  CheckAccount(){
        boolean flag = false;
        try{
            String sid = et_name.getText().toString();
            String pwd = et_password.getText().toString();
            OkHttpClient client = new OkHttpClient();
            String url = SQLUtil.serverhost+URLConfigUtil.getURLCheckAccount()+"/"+Base64Utils.encryptBASE64(sid)+"/"+Base64Utils.encryptBASE64(pwd);
    ...
    private class WeatherTask extends AsyncTask<String, Void ,String> {
        ...
        protected void onPostExecute(String value){
            ...
            try {
                JSONArray tem = JSONArray.parseArray(value);
                SharedPreferencesUtils helper = new SharedPreferencesUtils(LoginActivity.this,"setting");
                        helper.putValues(new SharedPreferencesUtils.ContentValue("stuid", tem.getJSONObject(0).getString("stuid")),new SharedPreferencesUtils.ContentValue("firname", tem.getJSONObject(0).getString("firname")),
                                new SharedPreferencesUtils.ContentValue("surname", tem.getJSONObject(0).getString("surname")),new SharedPreferencesUtils.ContentValue("fsport", tem.getJSONObject(0).getString("fsport"))
                        ,new SharedPreferencesUtils.ContentValue("fmovie", tem.getJSONObject(0).getString("fmovie")),new SharedPreferencesUtils.ContentValue("gender", tem.getJSONObject(0).getString("gender"))
                        ,new SharedPreferencesUtils.ContentValue("course", tem.getJSONObject(0).getString("course")),new SharedPreferencesUtils.ContentValue("stumode", tem.getJSONObject(0).getString("stumode")),
                                new SharedPreferencesUtils.ContentValue("suburb", tem.getJSONObject(0).getString("suburb")),new SharedPreferencesUtils.ContentValue("nation", tem.getJSONObject(0).getString("nationality")),
                                new SharedPreferencesUtils.ContentValue("language", tem.getJSONObject(0).getString("nalanguage")));
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        protected String doInBackground(String... strings) {
            ...
            try {
                OkHttpClient client = new OkHttpClient();
                String url = SQLUtil.serverhost+URLConfigUtil.getURLfindByStuid()+"/" + strings[0];
                Log.d("TestActivity", url);
                Request request = new Request.Builder().url(url).header("User-Agent", "OkHttp Headers.java")
                        .addHeader("Accept", "application/json; q=0.5")
                        .addHeader("Accept", "application/vnd.github.v3+json").build();
